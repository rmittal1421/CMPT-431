...
void triangleCountParallel(Graph &g, int& world_rank, int& P, int strategy) {
    if(strategy < 1 || strategy > 3) return;
    ...
    if(world_rank == ROOT_PROCESSOR) {
        ...
        printf("Number of unique triangles : %ld\n", global_count / 3);
...
int main(int argc, char *argv[])
{
    ...
    // If the strategy is 0, it should be run by one processor, and I am running it with root processor!
    if(strategy == 0 && world_rank == ROOT_PROCESSOR) {
        triangleCountSerial(g);
    } else {
        // Call the triangle counting function with appropriate strategy!
