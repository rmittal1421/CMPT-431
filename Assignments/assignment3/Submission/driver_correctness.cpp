...
bool serialTest(int n)
{
    // Add n elements and retrieve them
    for (int i = 0; i < n; i++)
    {
        std::cout << "Enqueue " << i << "\n";
        my_queue.enqueue(i);
    }
    std::cout << "Enqueued count : " << n << "\n";
    ...
    std::cout << "Dequeued count : " << dequeued_count << "\n";
}
...
int main(int argc, char *argv[])
{
    ...
    options.add_options("custom", {
                                      ...
                                      {"input_file", "Input file path for random numbers file", cxxopts::value<std::string>()->default_value("/scratch/assignment3/inputs/rand_10M")},
                                  });
